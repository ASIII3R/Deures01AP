...
public class Exercici0 {
    ...
    static{
        HashMap<String,Object> client100 = new HashMap<>();
        client100.put("nom","Joan García");
        client100.put("edat",45);
        client100.put("factors",Arrays.asList("autònom","risc mitjà"));
        client100.put("descompte",15);
        clients.put("client100",client100);

        HashMap<String,Object> client401 = new HashMap<>();
        client401.put("nom","Marta Pérez");
        client401.put("edat",38);
        client401.put("factors",Arrays.asList("empresa","risc baix"));
        client401.put("descompte",10);
        clients.put("client401",client401);

        HashMap<String,Object> client202 = new HashMap<>();
        client202.put("nom","Pere López");
        client202.put("edat",52);
        client202.put("factors",Arrays.asList("autònom","risc alt"));
        client202.put("descompte",5);
        clients.put("client202",client202);
    ...
    public static boolean validarNom(String nom) {
        ...
        String caractersCorrectes = " abcdefghijklmnopqrstuvwxyzàáèéìíòóùúäëïöüç";
        for (char car:nom.toCharArray()){
            if (caractersCorrectes.indexOf(car)==-1){
                return false;
            }
        }
        return true;
    ...
    public static boolean validarFactors(String[] factors) {
        if (factors != null && factors.length == 2){
            ...
            if ("autònom".equals(tipus)&& "risc baix".equals(risc)){
                return false; //Si es un autònom, no pot ser risc baix
            }
            return ("autònom".equals(tipus)|| "empresa".equals(tipus))&&
            ("risc alt".equals(risc)|| "risc mitjà".equals(risc) || "risc alt".equals(risc));
        }
        return false;
    }
    ...
    public static boolean validarData(String data) {
        ...
        if (!isAllDigits(dia)|| !isAllDigits(mes) || !isAllDigits(any)){
            return false;
        }
    ...
    public static String afegirOperacio(
        ...
        String nuevoID = generaClauOperacio();

        HashMap<String,Object> operacio = new HashMap<>();
        operacio.put("id",nuevoID);
...
Impostos:  21% (14.41)                     Total: 83.04
    ...
    public static ArrayList<String> taulaOperacionsClient(String clauClient, String ordre) {
        ...
        try {
            ...
            ArrayList<Object[]> columnesCapcelera = new ArrayList<>();
            columnesCapcelera.add(new Object[]{nomEdat,"left",25});
            columnesCapcelera.add(new Object[]{factors,"right",30});
            linies.add(alineaColumnes(columnesCapcelera));
            ...
            double preuDescomptat = sumaPreus * (percentatge/100);
